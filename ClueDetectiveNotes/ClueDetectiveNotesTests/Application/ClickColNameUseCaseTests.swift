...
final class ClickColNameUseCaseTests: XCTestCase {
    ...
    func test_whenClickAColnameThatPreviousSelectedInPreinferenceModeThenClearSelectedColname() {
        ...
        _ = clickColNameUseCase.execute(targetColName)
    ...
    func test_whenClickAColnameThatPreviousSelectedInInferenceModeThenClearSelectedColname() {
        let cells = sheet.getCells()
        let targetColName = cells[0].getColName()
        _ = sheet.selectColumnName(targetColName)
    ...
    func test_선택된ColName이없을때_ColName클릭하면_selectedColName이해당ColName이된다() {
        let cells = sheet.getCells()
        let targetColName = cells[0].getColName()
    ...
    func test_RowName이나ColName이1개이상선택되었을때_추리전모드로변경된다() {
        let cells = sheet.getCells()
        let targetColName = cells[0].getColName()
    ...
    func test_RowName이3개ColName1개선택되었을때_추리모드로변경된다() {
        let cells = sheet.getCells()
