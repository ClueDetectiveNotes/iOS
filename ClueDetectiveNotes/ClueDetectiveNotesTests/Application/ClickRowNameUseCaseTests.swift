...
final class ClickRowNameUseCaseTests: XCTestCase {
    ...
    func test_같은rowtype안에서이미선택된RowName이있을때_해당RowName을클릭하면_RowName의타입에해당하는selectedRowNames의값이nil이된다() {
        ...
        _ = clickRowNameUseCase.execute(targetRowName)
    ...
    func test_같은rowtype안에서선택된RowName이없을때_RowName을클릭하면_RowName의타입에해당하는selectedRowNames의값이해당RowName이된다() {
        let cells = sheet.getCells()
        let targetRowName = cells[0].getRowName()
        let targetRowNameType = cells[0].getRowName().card.type
    ...
    func test_RowName이3개ColName1개선택되었을때_추리모드로변경된다() {
        ...
        _ = clickRowNameUseCase.execute(targetRowName3)
